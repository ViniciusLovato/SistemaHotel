	...
	public void doPost (HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			if ((request.getParameter("email") == null) || (request.getParameter("senha") == null)){
				...
				// Abre sessao com o banco de dados
				Session sessionBD = sessionFactory.openSession();
				Transaction tx = sessionBD.beginTransaction();

				// Busca usuario com email informado no banco de dados
				usuario = (Usuario) sessionBD.load(Usuario.class, email);
				...
				*/
			    if (usuario.getEmail().equalsIgnoreCase(email)) {
					...
					if (!usuario.getSenha().equals(senha)){
						...
						sessionBD.update(usuario);

						// faz commit das alteracoes feitas
						tx.commit();
